...
def gatesJSON(data):
    from ..features.dignity import calculate_dignity, load_dignity_data

    dignity_data = load_dignity_data()

    # gate -> planet lookup per label, for finding the harmonic planet
    gate_to_planet = {}
    for i in range(len(data['label'])):
        lbl = data['label'][i]
        gate = data['gate'][i]
        gate_to_planet.setdefault(lbl, {}).setdefault(gate, data['planets'][i])

    # gate -> all planets across both labels, for gate-level fixing
    gate_to_all_planets = {}
    for i in range(len(data['label'])):
        gate_to_all_planets.setdefault(data['gate'][i], []).append(data['planets'][i])
    ...
    for i in range(len(data['label'])):
        ...
        dignity_result = calculate_dignity(
            ...
            harmonic_gate=ch_gate if ch_gate else None,
            harmonic_planet=harmonic_planet,
            dignity_data=dignity_data,
            gate_level_planets=gate_to_all_planets.get(gate, [])
        )
